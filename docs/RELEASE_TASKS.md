...
Проблема: кнопка получения награды в `QuestView` визуально не подходит к стилю окна и выбивается из
остального Journal/Quest UI.

Что сделать:
- Найти view/prefab-кнопку, которая отвечает за claim награды в `QuestView`.
- Обновить визуальный стиль кнопки так, чтобы она совпадала с текущим стилем Journal/Quest UI: размер,
  цвет, шрифт, иконка/текст, состояния normal/hover/pressed/disabled.
- Проверить, что кнопка всё ещё явно читается как основное действие для квеста в состоянии ReadyToAward.
- Проверить состояния: квест не готов к награде — кнопка скрыта/disabled как сейчас; квест готов —
  кнопка доступна; после claim — больше не предлагает получить награду.

Критичность: medium. Это visual polish, но на релизном Journal/Quest экране текущая кнопка выглядит
незаконченно.
...
Баг: в окне активного запроса кнопки закрытия и отмены выбора отключаются после первого использования и
остаются отключёнными при следующем открытии окна.

Что сделать:
- Найти, где окно активного запроса отключает кнопки Close / Cancel после клика или во время обработки выбора.
- При каждом открытии окна явно возвращать эти кнопки в интерактивное состояние.
- Проверить, что во время текущей операции кнопки всё ещё можно временно блокировать, но состояние не протекает
  в следующий показ окна.
- Добавить regression test или manual smoke: открыть active request, нажать Close/Cancel, открыть окно снова —
  обе кнопки доступны.

Критичность: high. Баг ломает повторное использование окна и может оставить игрока без понятного выхода из
мини-игры рекомендации.

### DEBUG-1 — Active Request Book Parameters Debug Button

Контекст: для визуального дебага активных запросов нужен быстрый способ посмотреть, почему конкретная книга
подходит или не подходит под текущий request.

Что сделать:
- В меню активного запроса добавить на каждую книгу debug-кнопку просмотра параметров.
- По клику открывать окно/панель со всеми параметрами выбранной книги, которые участвуют или могут участвовать
  в условиях запроса: жанры, качества, год публикации, страницы, теги/прочие релевантные поля.
- В этом же окне явно показывать результат проверки для текущего активного запроса: подходит книга или нет.
- Если книга не подходит, показывать какие условия не прошли, чтобы можно было визуально сверить config и UI.
- Держать фичу как dev/debug tooling: она нужна для проверки контента и условий, а не как финальная
  player-facing механика релиза.
- Проверить manual smoke: открыть active request, нажать debug-кнопку на подходящей и неподходящей книге,
  увидеть параметры книги и корректный match / mismatch по текущему запросу.

Критичность: medium. Это ускоряет проверку request configs и балансировку, но не является обязательной
игровой механикой для игрока.

### BUG-2 — PreparationWindow Available Book Count Ignores Selected Books

Баг: в `PreparationWindow` количество книг "в наличии" не уменьшается на уже выставленные экземпляры.
Игрок может видеть полный stock, хотя часть книг уже добавлена на полку текущей подготовки.

Что сделать:
- Найти источник данных для строк книг в `PreparationWindow` и место, где считается количество owned/available.
- При отображении и обновлении списка показывать доступное количество как `ownedCount - selectedCount` для каждой книги.
- Учитывать изменения выбора в текущей сессии подготовки: добавили книгу на полку — доступное количество сразу уменьшилось; убрали с полки — увеличилось.
- Не менять фактический inventory до confirm: это только расчёт доступности внутри preparation-сессии.
- Добавить regression test или manual smoke: при наличии N экземпляров книги и выборе K экземпляров окно показывает N-K доступных, не даёт выбрать больше N, а после отмены/снятия выбора количество восстанавливается.

Критичность: high. Ошибка путает игрока и может позволить собрать полку, которая не соответствует реальному
количеству книг в инвентаре.

### REL-16 — Add Reset All Button To PreparationWindow

Что сделать:
- Добавить в `PreparationWindow` кнопку "Сбросить всё" для очистки текущего выбора книг.
- По клику сбрасывать все выбранные книги / genre quantities текущей preparation-сессии к 0.
- Не менять фактический inventory и не подтверждать день: это только сброс выбора до confirm.
- После сброса обновлять строки, счётчик слотов и состояние кнопки Start.
- Проверить manual smoke: выбрать несколько книг, нажать "Сбросить всё" — все строки 0, счётчик 0/N,
  Start снова в корректном состоянии; после этого повторный выбор книг работает.

Критичность: medium. Это UX-удобство для окна подготовки, особенно когда игрок хочет быстро пересобрать полку
без ручного минусования всех строк.
