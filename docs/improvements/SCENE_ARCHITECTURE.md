...
> ⚠️ **Частично superseded — см. [GameFlowLoop.md](../GameFlowLoop.md).**
>
> Структура сцен и режим перехода пересмотрены. Источник истины по сценам/переходам — `GameFlowLoop.md`.
> Конкретно отменено:
> - **§3** (рекомендация переименовать `GameplayScene`→`LocationScene` + новый `HubScene`). Финальное
>   решение: `GameplayScene` **остаётся хабом**, добавляется **новая** `LocationScene`.
> - **§8 антипаттерн «additive поверх»**. Финальное решение — именно **additive**: хаб остаётся
>   загружен, `LocationScene` грузится поверх и выгружается при возврате.
>
> Разделы про **окна хаба** (§4), **diegetic-точки** (§7) и **импакт на код** (§6) остаются актуальными.
