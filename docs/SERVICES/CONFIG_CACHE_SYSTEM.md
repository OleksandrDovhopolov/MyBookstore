...
|---|---|---|
| `LocalFolderConfigSource` | Editor по умолчанию; bundled-слой `ServerConfigSource` в Editor | Читает `Assets/Configs/*.json`. Правка JSON → Play → видно сразу |
| `StreamingAssetsConfigSource` | Bundled-слой `ServerConfigSource` в Build | Читает `StreamingAssets/Configs/` через `UnityWebRequest` (на Android прямой `File.IO` не работает — путь внутри APK jar). Список файлов — из `manifest.json` |
...
**Регламент bundled defaults:** перед каждым релизным билдом запустить `Tools/Configs/Sync Bundled Defaults to StreamingAssets` — копирует `Assets/Configs/*.json` в `Assets/StreamingAssets/Configs/` и регенерирует `manifest.json`. Без этого свежий install + оффлайн стартует с пустым каталогом.
...
| Editor-меню синхронизации | `Assets/Game/Features/Configs/Editor/SyncBundledDefaultsMenu.cs` |
