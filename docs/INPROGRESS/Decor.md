...
- **8 слотов в первой локации** — data-driven через `LocationConfig.DecorSlots`
...
**Точка размещения в магазине**. Описана в `LocationConfig.DecorSlots`:
...
В первой локации (`loc_downtown`) Phase 0 даёт **8 слотов**:
...
### 4.2 LocationConfig — расширение

```
[ConfigFile("locations")]
public sealed class LocationConfig : IConfig {
    // ... existing fields (Id, DemandGenres, DemandTags) ...
...
## 9. Sample slots в LocationConfig

Расширение `locations.json` для `loc_downtown`:
...
- **`ICurrentLocationProvider`** — в Phase 0 захардкодить `"loc_downtown"` в `DecorPlacementService`. Phase 1+ — нормальный сервис когда появится UI выбора локации.
...
Assets/Game/Features/Configs/Models/LocationConfig.cs                              ← +DecorSlot[] DecorSlots
...
Assets/Configs/locations.json                                                       ← +decorSlots array on loc_downtown
...
### LocationConfig.DecorSlots

| Правило | Действие |
|---|---|
| `Slot.Id` непустой и уникальный в рамках одной локации | Hard fail |
...
| Каждый `Placement.SlotId` ссылается на существующий `Slot` в текущей `LocationConfig` | Если orphan → удалить запись + LogWarning |
