...
## 5. Scene-инсталлеры: GameInstaller (хаб) и LocationInstaller (локация)

Игровой цикл разделён на **две сцены** (см. [GameFlowLoop.md](GameFlowLoop.md)): хаб `GameplayScene` (базовая)
и `LocationScene` (Sales-день, грузится additive). У каждой — свой scene-скоуп и `MonoInstaller`, оба —
дети `GlobalLifetimeScope`.

### GameInstaller (хаб) — `Assets/Game/Core/Installers/Bootstrap/GameInstaller.cs`
`MonoInstaller` на `GameplayLifetimeScope`. Регистрирует hub-сервисы:
```csharp
...
// + scene-компоненты хаба, если есть в сцене:
//   RegisterComponentInHierarchy<HubRootBinder>()   — гасит/включает GameplayHubRoot
//   RegisterComponentInHierarchy<HubPhaseRouter>()  — визуальное состояние хаба по фазе дня
```

### LocationInstaller (локация) — `Assets/Game/Core/Installers/Bootstrap/LocationInstaller.cs`
`MonoInstaller` на `LocationLifetimeScope` (в `LocationScene`). Регистрирует sales-сервисы:
```csharp
builder.Register<ISalesSetupProvider, PreparationSalesSetupProvider>(...); // читает preparation.session из Save
builder.RegisterBookSell(/* customer anchors + SalesTuningConfig */);      // SalesDayController, спавнер, визуалы
```

> `ISalesShelfStateService` — **общий** для хаба (Preparation) и локации (Sales), поэтому зарегистрирован
> **глобально** в `BootstrapInstaller` (`RegisterBookSellSharedState`), а не в location-скопе.

> Многие «сцено-специфичные» сервисы из прежней версии (`Resources`, `Inventory`, `Shop`, …) на деле
> **глобальные** и регистрируются в `BootstrapInstaller` (раздел 4 / диаграмма 11).
...
Полная картина игрового цикла и переходов — в [GameFlowLoop.md](GameFlowLoop.md). Кратко:

### Текущий статус

- **`LoadingOrchestrator`, `LoadingProgressAggregator`, `LoadingScreenView`** — реализованы. См. раздел 6.
- **`ISceneTransitionService`** — реализован и расширен: `TransitionToAsync` (Single, для boot→game) +
  `LoadAdditiveAsync` / `UnloadAsync` / `SetActiveScene` (для цикла хаб ↔ локация).
- **`IGameFlowService` / `GameFlowService`** — оркестрирует цикл `GameplayScene (хаб) ↔ LocationScene (additive)`:
  `EnterLocationAsync` / `ReturnToHubAsync`. Родителем `LocationLifetimeScope` назначается `GlobalLifetimeScope`
  через `LifetimeScope.EnqueueParent(global)`.
- **`ITransitionAnimationService`** — есть как **хук** (`NoOpTransitionAnimationService`); реальный cover/reveal
  (своим UI-кодом, без DOTween) — отдельной задачей.
- **`HubPhaseRouter`** — восстанавливает визуал хаба по `IDayProgressService.PhaseChanged` (Next Day без reload сцены).

### UI System — реализовано (не стабы)

`UIManager` + `IWindowFactory` (`AddressablesWindowFactory`) — реализованы (см. [UI_SYSTEM.md](UI_SYSTEM.md)).
Окнам со scene-scoped зависимостями данные передаются через `WindowArgs` (как `RecommendationMinigameWindow`),
а не через scene-resolver. `IWindowRouter` / nav stack и полноценный cover/reveal — ещё в roadmap.
...
GlobalLifetimeScope (DontDestroyOnLoad)
    ...
    │
    ├── GameplayLifetimeScope (хаб, child scope — GameplayScene)
    │       └── GameInstaller (MonoBehaviour)
    │               ├── RegisterDayCycleSceneViews()
    │               ├── RegisterRewardDrop() / RegisterIap() / RegisterQuest()
    │               ├── RegisterPreparation()
    │               └── HubRootBinder + HubPhaseRouter (RegisterComponentInHierarchy)
    │
    └── LocationLifetimeScope (локация, child scope — LocationScene, additive)
            └── LocationInstaller (MonoBehaviour)
                    ├── ISalesSetupProvider (PreparationSalesSetupProvider)
                    └── RegisterBookSell()  (SalesDayController, спавнер, визуалы)
```

> Родителем `LocationLifetimeScope` назначается `GlobalLifetimeScope` в рантайме (`GameFlowService` →
> `LifetimeScope.EnqueueParent`), а не `GameplayLifetimeScope`. `ISalesShelfStateService` — глобальный
> (`RegisterBookSellSharedState` в `BootstrapInstaller`), общий для обоих child-скопов.
...
| Реальная cover/reveal-анимация (своим UI-кодом) под `ITransitionAnimationService` | `Game.UI` | Низкий |
...
> `ITransitionAnimationService` (no-op хук), `MainSceneBootstrap` (в `GameplayScene`),
> `ISceneTransitionService` additive + `IGameFlowService` (цикл хаб ↔ локация).
