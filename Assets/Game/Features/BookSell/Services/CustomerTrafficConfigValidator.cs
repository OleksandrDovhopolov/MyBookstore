...
namespace Book.Sell.Services
{
    ...
    /// below the number of active RequestConfigs, some scripted active requests would go unserved — warn the
            ...
            foreach (var day in _configs.GetAll<DayConfig>())
            {
                ...
                if (day.CustomerCount.Value < requestCount)
                {
                    warnings.Add(
                        ...
                        $"{day.CustomerCount.Value}, below the {requestCount} active RequestConfig(s). " +
