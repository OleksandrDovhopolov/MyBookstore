...
{
    /// <summary>In-memory <see cref="IInventoryService"/> fake for shop tests. Mirrors the one in
    /// Rewards.Tests; isolated copy keeps asmdef boundaries clean.</summary>
    internal sealed class FakeInventoryService : IInventoryService
    {
        private readonly List<InventoryItem> _items = new();

        public List<(string id, string category, int amount)> AddCalls { get; } = new();
